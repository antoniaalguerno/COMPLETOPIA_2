...
export const EditProduct: React.FC = () => {
    ...
    const fetchProduct = async () => {
      ...
      try {
        const response = await fetch(
          `http://localhost:8000/api/inventario/products/${productId}/`,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/api/inventario/products/${productId}/update/`,
        {
          method: 'PUT',
          headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errData.detail || errData.error || 'Falló la actualización'
        );
      }
  ...
  if (error) {
    return (
      <div className="edit-product-page" style={{ color: 'red' }}>
        {error}
      </div>
    );
  }
